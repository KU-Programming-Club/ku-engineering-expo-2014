...
import edu.kuacm.expo2014.AsyncActivityTask;
import edu.kuacm.expo2014.ExpoApplication;
...
public class ScoresFragment extends Fragment {
	...
	public static class LoadScoresTask extends AsyncActivityTask<FragmentActivity, Integer, Void, JSONObject> {
		private static final String TEAM_SCORES_URL = "http://expo.zapto.org/api/getTeamScore/";
		...
		protected void onPostExecute(JSONObject result) {
			...
			try {
				...
				if (status != 0) {
					...
					getFragment().showToast("Invalid ID.");
				}
			} catch (JSONException e) {
				e.printStackTrace();
